...
function Experience() {
  ...
  const getCertificateImage = (exp) => {
    ...
    if (exp.id === 2) {
      const imagePath = "/certificates/Unovative_Prathamesh_Internship_Experience_Certificate.jpg";
      return imagePath;
    }
    return exp.pic;
  };
        ...
        >
          ...
          <div className="flex justify-between items-center gap-2 mb-4">
            <div className="flex gap-2">
              ...
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 8v6m3-3H7" />
                </svg>
              </button>
          ...
          <div className={`relative flex-1 bg-gray-900 rounded-lg overflow-hidden shadow-2xl ${zoomLevel > 1 ? 'overflow-auto' : ''}`}>
            {/* Loading State */}
            {imageLoading && (
              <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-90 z-10">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-gray-300 text-sm font-medium">Loading certificate...</p>
                </div>
              </div>
            )}
          ...
          <div className="bg-gradient-to-r from-gray-900 to-black p-4 rounded-b-lg mt-2 border-t border-gray-700">
            ...
            <p className="text-gray-300 text-lg">{selectedCertificate.company}</p>
            <p className="text-[#16f2b3] text-sm font-medium">{selectedCertificate.duration}</p>
          </div>
        </div>
      </div>,
      document.body
            ...
            <div className="flex flex-col gap-6">
              {
                experiences.map(experience => (
                  <GlowCard key={experience.id} identifier={`experience-${experience.id}`}>
                    <div 
                      className={`p-3 relative ${experience.pic ? 'cursor-pointer hover:bg-opacity-80 transition-all duration-200' : ''}`}
